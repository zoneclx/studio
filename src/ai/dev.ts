...
const createWebsiteFromPromptPrompt = ai.definePrompt({
...
export const createWebsiteFromPromptFlow = ai.defineFlow(
  {
  ...
  async input => {
    const { output } = await createWebsiteFromPromptPrompt(input);
    if (!output) {
